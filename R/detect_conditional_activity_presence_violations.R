...
detect_conditional_activity_presence.activitylog <- function(activitylog, condition, activities, details = TRUE, filter_condition = NULL){
  ...
  if (length(violated) != 0) {
    print(combined_process_map_cond_act(activitylog, filter_case(activitylog, violated, reverse = T))) %>% print()
  }
  # NEW Section >>>>>>>>>>>>>>>>>> END
...
combined_process_map_cond_act <- function(activitylog_1, activitylog_2) {
  ...
  header <- tags$div("From Top to Bottom: Process map of All Cases and Cases Holding The Condition",
