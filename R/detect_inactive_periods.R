...
detect_inactive_periods.activitylog <- function(activitylog,
  ...
  if(timestamp == "both") {
    log0 <- log %>% select(period_start = prior_complete, period_end = start, time_gap)
  } else if (timestamp == "start") {
    log0 <- log %>% select(period_start = prior_start, period_end = start, time_gap)
  } else if (timestamp == "complete") {
    log0 <- log %>% select(period_start = prior_complete, period_end = complete, time_gap)
  }
  
  if (type == "arrivals") {
    trend_arrival_ggplot_inact_per(activitylog, log0 , start_activities) %>% print()
    
  } else if (type == "activities") {
    duration_activity_ggplot_inact_per(activitylog, log0) %>% print()
  }
...
trend_arrival_ggplot_inact_per <- function(activitylog, log, start_activities) {
  start <- NULL
  period_start <- NULL
  period_end <- NULL
  start_case <- NULL
  ...
  if (!is.na(start_activities)) {
    activitylog <- activitylog %>% filter_activity(start_activities)
  }
  
  
  df <- activitylog %>% data.frame() %>%
    group_by(!!case_id_(activitylog)) %>%
    summarise(start_case = min(start, na.rm = T))
  
  colnames(df)[1] <- "case_id"
  
  
  
  ggplot(df, aes(x = start_case, y = case_id)) +
    geom_point(na.rm = TRUE, color = "#253D58") +
    xlab("") +
    geom_line(color = "#253D58", na.rm = TRUE) +
    geom_rect(
      data = log,
      fill = "red4",
      alpha = 0.1,
    ...
    labs(y = "Case ID", x = "Starting Time", title = "Inactive Periods (Red Sections)") +
    theme(
...
duration_activity_ggplot_inact_per <- function(activitylog, log) {
  start <- NULL
  complete <- NULL
  activity <- NULL
  period_start <- NULL
  ...
  df <- activitylog %>%
    data.frame() %>% select(
      ...
      activity = activity_id(activitylog),
      start  ,
      complete
    )
  ...
  ggplot_object <- ggplot(df) +
    ...
    labs(
      y = "Active Time",
      x = "Case ID",
      title = "Inactive Periods (Red Section)",
      color = "Activity"
    ) +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1),
